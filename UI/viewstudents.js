...
const getStudents = async(email) => {
    const students = await fetch(encodeURI(`http://${api_url}:${port}/api/students/${email}`), {
    mode: "cors"
...
const populateStudentList = async () =>{
    const students = await getStudents(email)
    students.map((student)=>{
        ...
        studentButton.addEventListener("click", ()=>{
            ...
            localStorage.setItem("selectedStudent", student.IDnumber)
